...
class TestDjUserReadRepository(TestCase):
    ...
    def test_get_all_without_filter(self):
    ...
    def test_get_all_with_username_filter(self):
        result: list[User] = self.read_repo.get_all(
...
class TestDjUserWriteRepository(TestCase):
    ...
    def test_update_user_all_fields(self):
        ...
        self.assertTrue(result.email == user.email == data.email.value)
        self.assertTrue(result.username == user.username == data.username.value)
        self.assertEqual(result.password, user.password)
        self.assertTrue(check_password(data.password.value, result.password))
    ...
    def test_update_password_field(self):
        data = UserUpdateData(
